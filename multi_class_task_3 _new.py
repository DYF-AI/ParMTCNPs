...
class TrainNet:
    ...
    def set_param(self, code_num, decode_num):
        ...
        # 解码器第五层方差
        self.w_fc325 = tf.Variable(tf.truncated_normal([TrainNet.CON_LAYERS, releate_node], stddev=0.1))
        self.b_fc325 = tf.Variable(tf.constant(0.1, shape=[releate_node]))
        ...
        if add_layer:
            self.w_fc11_add = tf.Variable(tf.truncated_normal([task_num*releate_node, releate_node], stddev=0.1))
            self.b_fc11_add = tf.Variable(tf.constant(0.1, shape=[releate_node]))
            self.w_fc12_add = tf.Variable(tf.truncated_normal([task_num*releate_node, releate_node], stddev=0.1))
            self.b_fc12_add = tf.Variable(tf.constant(0.1, shape=[releate_node]))
            ...
            self.w_fc21_three = tf.Variable(tf.truncated_normal([releate_node, 1], stddev=0.1))
            self.b_fc21_three = tf.Variable(tf.constant(0.1, shape=[1]))
            ...
            self.m_two = tf.matmul(self.h_fc11_relu, self.w_fc21_three) + self.b_fc21_three
            
            self.m_one = tf.reshape(self.m_one, [-1, self.pointsDecodeTrainNums, 1])
            self.m_two = tf.reshape(self.m_two, [-1, self.pointsDecodeTrainNums, 1])
            self.m_three = tf.reshape(self.m_two, [-1, self.pointsDecodeTrainNums, 1])
            ...
            self.w_fc23_three = tf.Variable(tf.truncated_normal([releate_node, 1], stddev=0.1))
            self.b_fc23_three = tf.Variable(tf.constant(0.1, shape=[1]))
            ...
            self.log_v_three = tf.matmul(self.h_fc12_relu, self.w_fc22_three) + self.b_fc22_three
    ...
    def cnp_train_model_1(self, observe_point_one, total_point_x_one, total_point_y_one, observe_point_two, total_point_x_two, total_point_y_two, observe_point_three, total_point_x_three, total_point_y_three):
        ...
        if add_layer:
            self.param_dict['fc_11w_add'] = self.w_fc11_add
            self.param_dict['fc_11b_add'] = self.b_fc11_add
            self.param_dict['fc_12w_add'] = self.w_fc12_add
            self.param_dict['fc_12b_add'] = self.b_fc12_add
